...
Gemini CLI, Codex, or any other tool. `AGENTS.md` is the cross-agent filename
convention, so this is the only context file, wherever it sits. There is no
`GEMINI.md`.

A `CLAUDE.md` sits at the project root as a stub holding nothing but a pointer
here, because Claude Code auto-loads that filename and no other. Never add
content to the stub. Tools that read neither name should be pointed at this
file explicitly, in an opening prompt or the project README. For where each
file lives in each layout, master source versus downstream copy, see
